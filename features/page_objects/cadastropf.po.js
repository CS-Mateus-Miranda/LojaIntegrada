...
class CadastroPF{
  construtor(){
    this.email = element(by.id('id_email')).sendKeys('miranda@miranda.com');
    this.btnConfirmar = element(by.css('#id_email+ .principal')).click();
    browser.wait(started, 8 * 1000, 'Server should start within 5 seconds');
  }

  cadastrocompleto(){
    this.inputconfirmemail = element(by.id('id_confirmacao_email')).sendKeys('miranda@miranda.com');
